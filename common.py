...
def auth_headers() -> dict:
    """로그인 뒤 모든 요청에 붙이는 헤더."""
    return {"Authorization": "Bearer expired.token.value"}
...
def api(method: str, path: str, **kwargs):
    ...
    try :
        res = httpx.request(method, f'{BACKEND_URL}{path}'
                              , timeout=HTTP_TIMEOUT
                              , **kwargs)
    except httpx.ConnectError:
        raise ApiError("백엔드 서버에 연결할 수 없습니다. ")
...
def auth_headers() -> dict:
    ...
    # print(f"Bearer {st.session_state.access_token}")
