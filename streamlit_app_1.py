...
from common import ApiError, SessionExpired, api, auth_headers, conversation_label,SERVICE_NAME, stream_answer

# 상태종류
# 1. 비로그인
# 2. 로그인 + 대화목록 없음
# 3. 로그인 + 대화목록 있음 + 대화선택 안함 > 제일 위의 대화 선택
# 4. 로그인 + 대화목록 있음 + 대화선택 있음 > 메세지 없음 > 예시질문 출력
# 5. 로그인 + 대화목록 있음 + 대화선택 있음 > 메세지 있음 > 대화화면 출력 + 꼬리질문 출력
...
def render_sidebar(options: dict, conversations: list) -> None:
    with st.sidebar :
        ...
        # try :
        #     conversations  = api("GET", "/conversations", params={"user_id": st.session_state.user_id})
...
def render_example(conversation_id: str) -> None:
    ...
    icons = ["🚀", "🙋", "🔍"]
    for col, question, icon in zip(columns, EXAMPLE_QUESTIONS, icons):
        #클릭한 버튼의 질문을 세션 변수에 저장합니다.
        if col.button(f"{icon}  {question}", use_container_width=True):
            st.session_state.pending_question = question
            st.rerun()
...
def render_login() -> None:
    ...
    print('***'+st.session_state.expired_notice)
...
# 브라우저 화면에 렌더링하는 영역        
    ...
    if not conversations :
        ...
        render_conversation(st.session_state.conversation_id, options["max_history_messages"])
